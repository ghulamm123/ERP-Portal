...
if($_SESSION['teamlead_A_order']==1)
	{
...
$sent_by="--";
$time="--";
$sql = "SELECT * FROM `orders` WHERE reciver_ID='teamlead_A' ";
		$result = $conn->query($sql);
...
<section>
    ...
    <div class="layer1">
        ...
        <h2> "A" TEAM LEAD VIEW </h2>
        <input type="submit" id="gt" value="GIVE TASK"onclick="give_task()"> 
		<input type="submit" id="gr" value="MESSAGE"onclick="order()">
        <input type="submit" id="gr" value="EMP:TASK" onclick="see_task()"> 
...
<?PHP				
		$sql = "SELECT * FROM project where team='A' or team='AB' ";
		$result = $conn->query($sql);
		if ($result->num_rows > 0) {
    // output data of each row
						while($row = $result->fetch_assoc())
...
function give_task()
{
    ...
    window.location.replace("givetaskteamleadA.php");
}
function see_task()
{
    
    window.location.replace("teamAemployess.php");
}
function order()
{
    
    window.location.replace("order_teamleadA.php");
}
</script>
